...
const users = [
  { id: 1, username: "alice", name: "Alice Rossi", password: "password", salt: "alice-salt" },
  { id: 2, username: "bruno", name: "Bruno Verdi", password: "password", salt: "bruno-salt" },
  { id: 3, username: "carla", name: "Carla Bianchi", password: "password", salt: "carla-salt" },
];

const stations = [
  [1, "Centrale", 1, 120, 90],
  [2, "Porta Velaria", 1, 260, 90],
  [3, "Crocevia del Falco", 0, 400, 90],
  [4, "Piazza delle Lanterne", 1, 540, 90],
  [5, "Fontana Oscura", 1, 260, 220],
  [6, "Borgo Sereno", 0, 400, 220],
  [7, "Viale dei Mosaici", 0, 540, 220],
  [8, "Torre Cinerea", 1, 400, 350],
  [9, "Campo dell'Eco", 0, 540, 350],
  [10, "Giardino del Sale", 0, 120, 220],
  [11, "Arco delle Nebbie", 0, 120, 350],
  [12, "Mercato Antico", 0, 260, 350],
];
...
try {
  ...
  for (const station of stations)
    await run("INSERT INTO stations(id, name, is_interchange, x, y) VALUES (?, ?, ?, ?, ?)", station);

  for (const line of lines)
    await run("INSERT INTO lines(id, name) VALUES (?, ?)", line);

  for (const segment of segments)
    await run("INSERT INTO segments(id, line_id, station_a_id, station_b_id) VALUES (?, ?, ?, ?)", segment);

  for (const event of events)
    await run("INSERT INTO events(id, description, effect) VALUES (?, ?, ?)", event);
